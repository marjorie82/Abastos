...
class KsSalesForecast(models.TransientModel):
    _name = 'ks.sales.forecast.result'
    _rec_name = 'ks_forecast_id'

    ks_forecast_id = fields.Many2one('ks.sales.forecast', ondelete="cascade")
    ks_date = fields.Date(string=_('Date'))
    ks_value = fields.Float(string=_('Sale'))
    ks_product_id = fields.Many2one('product.product', string='Product')
